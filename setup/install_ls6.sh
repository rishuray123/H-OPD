...
# Keep flash_attn OUT of this venv. vLLM 0.12 ships its own kernels, and its
# vllm_flash_attn imports flash_attn.cute, which explodes with
# `cutlass.cute.core has no attribute ThrMma` against the cutlass-dsl that vLLM
# pulls in. verl's padding helpers fall back to transformers on the hopd branch.
...
from verl.utils.attention_utils import unpad_input  # verl compute_log_prob tripwire
print("verl padding helpers OK")
PY
