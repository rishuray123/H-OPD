...
- 2026-09-21: Run observability: `HOPD_LOGGER` defaults to `["console","tensorboard","file"]` — **no W&B**. The key does nothing unless `HOPD_LOGGER` includes `wandb`. The `file` backend writes `$RUN_LOG_DIR/metrics.jsonl` (`VERL_FILE_LOGGER_PATH`), tensorboard goes to `$RUN_LOG_DIR/tensorboard` (`TENSORBOARD_DIR`), and `trainer.rollout_data_dir` / `validation_data_dir` dump one JSONL per step. `logs/mopd_latest` symlinks the current run. Read them with `experiments/mopd/watch.py --follow` (metric table + rollout accuracy) and `experiments/mopd/show_rollouts.py`. Smoke `TEST_FREQ` is -1; `MOPD_FULL=1` defaults to 50. Validation is a full pass over the val file (MathVista-200).
...
- 2026-09-24: `acc` is always computed in `task_reward.py` and dumped on every rollout. `HOPD_TASK_REWARD=1` does not turn tracking on — it puts `acc` into the PPO advantage. That requires GRPO groups; `run_ls6.sh` now raises `rollout.n` to 4 when the flag is set. Do not use `n=1` with task reward. For a cleaner acc curve without changing the loss, set `TEST_FREQ=50`.
...
- 2026-09-26: Paper H-OPD recipe is `HOPD_PAPER=1` / `bash experiments/mopd/run_lightning.sh paper`. That turns on `mix_teachers`, scores both teachers on the same \(Y\), mixes on \(\Omega_t=K_V\cup K_T\), and trains with `reverse_kl_topk` as a supervised loss (`use_policy_gradient=False`). The text teacher is scored without pixels and with Qwen VL image pads `{151652–151656}` stripped from the **prompt only**. `HOPD_MIX=1` stays the older \(p(y_t)\) + k1/PPO path. Need verl `hopd` after this change (`bootstrap_verl.sh`). Scale is still not the paper's 55k / 12384 / 128 / 8×B200.
- 2026-09-26: Paper smoke on Lightning (`hopd-mopd-smoke96-paper-local`, W&B `3afjd9sj`) loaded all three vLLMs and finished the first rollout, then died in `update_actor` with `KeyError: 'distillation_losses'`. `reverse_kl_topk` / `forward_kl_topk` compute that tensor in the FSDP logits processor, which only ran when `use_remove_padding=True`. This recipe sets it `False` (flash_attn). verl `hopd` now runs the same hook on the no-rmpad path. After `bootstrap_verl.sh`, rerun the same `HOPD_PAPER=1` command.
- 2026-09-26: Next paper smoke (`0ftxbo8c`) reached reverse KL and died with `Index tensor must have the same number of dimensions as input tensor`. `left_right_2_no_padding` used `index_first_axis` on `(B, S, K)` teacher tables; the transformers flash_attn fallback reshapes to `(T, last_dim)` and drops K, so gather saw `[1, nnz]` ids vs `[1, nnz, V]` logits. Unpad the top-k axis with a plain flatten+index instead. After `bootstrap_verl.sh`, rerun `HOPD_PAPER=1`.
