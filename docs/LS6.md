...
- 2026-09-21: After the schema fix, `probe_lengths` still hit `Cannot have both bytes and image` because `_build_messages` mutated the row: `image["image"] = Image.open(bytes)` while leaving `bytes` in the same dict, then `doc2len` called `process_image` on that dict and the filter treated the exception as overlong. Fixed on verl `hopd` (`9e0d8057` follow-up): copy the image dict, do not mutate the dataset row, and if `bytes` is present pop `image` before opening. Re-run `bash setup/bootstrap_verl.sh`; the parquet does not need to be rebuilt for this one.
