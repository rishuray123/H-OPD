...
# left_right_2_no_padding needs these on the first compute_log_prob call. On the
# hopd branch they come from transformers when flash_attn is absent, which is the
# supported setup: a standalone flash_attn breaks vllm_flash_attn's cute import.
try:
    from verl.utils.attention_utils import _get_attention_functions

    _get_attention_functions()
    line("verl padding helpers", "ok")
except Exception as e:  # noqa: BLE001
    line("verl padding helpers", f"FAILED {type(e).__name__}: {e}")
    BAD.append("verl padding helpers fail -> compute_log_prob dies at step 1")

try:
    import flash_attn  # noqa: F401

    line("flash_attn present", "yes — remove it if vllm_flash_attn/cute errors")
except ImportError:
    line("flash_attn present", "no (expected)")
